...
public class MailTemplateEditMenu extends BaseMenu {
    ...
    void setItems(@NotNull ClickRegistry clickRegistry) {
        ...
        final ItemStack sender = getSender();
        clickRegistry.setItem(1, sender, (player, clickType) -> {
            if (clickType.isLeftClick()) {
        ...
        startTime.editMeta(meta -> {
            ...
            meta.lore(List.of(miniMessage().deserialize(template.startTime() + "")));
        });
        clickRegistry.setItem(2, startTime, (player, clickType) -> {
            if (clickType.isLeftClick()) {
                openMenu(new AnvilSetterMenu(this, player, miniMessage().deserialize(config().messages.setStartTime), value -> {
                    template.setStartTime(LocalDateTime.parse(value, DateTimeFormatter.ISO_OFFSET_DATE_TIME));
                    refresh();
                }, Optional.ofNullable(template.startTime()).map(l -> l.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME)).orElse(LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME))));
            }
        });
        ...
        endTime.editMeta(meta -> {
            ...
            meta.lore(List.of(miniMessage().deserialize(template.endTime() + "")));
        });
        clickRegistry.setItem(3, endTime, (player, clickType) -> {
            if (clickType.isLeftClick()) {
                openMenu(new AnvilSetterMenu(this, player, miniMessage().deserialize(config().messages.setEndTime), value -> {
                    template.setEndTime(LocalDateTime.parse(value, DateTimeFormatter.ISO_OFFSET_DATE_TIME));
                    refresh();
                }, Optional.ofNullable(template.endTime()).map(l -> l.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME)).orElse(LocalDateTime.now().format(DateTimeFormatter.ISO_DATE_TIME))));
            }
        });
        ...
        interval.editMeta(meta -> {
            ...
            meta.lore(List.of(miniMessage().deserialize(Optional.ofNullable(template.interval()).map(d -> d.toHours() + "h" + d.toMinutesPart() + "m" + d.toSecondsPart() + "s").orElse("null"))));
        });
        clickRegistry.setItem(4, interval, (player, clickType) -> {
            if (clickType.isLeftClick()) {
        ...
        permissions.editMeta(meta -> {
            ...
            meta.lore(List.of(miniMessage().deserialize(String.valueOf(template.permission()))));
        });
        clickRegistry.setItem(5, permissions, (player, clickType) -> {
            if (clickType.isLeftClick()) {
        ...
        attachments.editMeta(meta -> {
            ...
            meta.lore(List.of(miniMessage().deserialize(template.attachment().size() + "")));
        });
    ...
    private @NotNull ItemStack getSender() {
        ...
        sender.editMeta(meta -> meta.customName(miniMessage().deserialize(config().messages.sender, TagResolver.builder().tag("sender", Tag.inserting(miniMessage().deserialize(template.sender() == null ? config().messages.systemName : Optional.ofNullable(Bukkit.getOfflinePlayer(template.sender().uuid()).getName()).orElse(template.sender().uuid().toString())))).build())));
        return sender;
    }
}
