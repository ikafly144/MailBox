...
public class MailTemplateMenu extends BaseMenu {
    ...
    public MailTemplateMenu(Player player, int page) {
        super(player, 45, miniMessage().deserialize(config().messages.mailTemplateMenuTitle + " " + page));
        this.page = page;
    }
    ...
    void setItems(@NotNull ClickRegistry clickRegistry) {
        ...
        clickRegistry.setItem(0, arrow, (player, clickType) -> {
            if (clickType.isLeftClick() && page > 1) {
                openMenu(new MailTemplateMenu(player, page - 1));
            }
        });
        ...
        clickRegistry.setItem(4, paper, (player, clickType) -> {
            if (clickType.isLeftClick()) {
                openMenu(new CreateMailMenu(player));
            }
        });
    ...
    private ItemStack createMailItem(MailTemplate mail) {
        ...
        item.editMeta(meta -> {
            ...
            meta.lore(config().messages.mailTemplateLore
                    ...
                    .replace("{start}", mail.startTime() + "")
                    .replace("{end}", mail.endTime() + "")
                    .replace("{interval}", Optional.ofNullable(mail.interval()).map(d -> DurationFormatUtils.formatDuration(d.toMillis(), "HH:mm:ss")).orElse("null"))
                    .replace("{attachments}", mail.attachment().size() + "")
                    .replace("{auto_send}", mail.autoSend() + "")
